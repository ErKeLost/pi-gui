...
import { request, report, refresh, changeSession, connect, disconnect, native, loadMessages, answerDialog, retireSession, getProjectTrustMode, setProjectTrustMode, type ProjectTrustMode } from '../lib/rpc'
...
export function Sessions(){
 const cwd=useWorkspace(s=>s.cwd),online=useWorkspace(s=>s.connection==='online'),liveSessions=useWorkspace(s=>s.liveSessions)
 ...
 async function remove(){if(!deleting)return;try{await retireSession(deleting.path);setDeleting(null);await sessions.refetch();gooeyToast.success('会话已删除',{showTimestamp:false})}catch(error){report(error)}}
 const listed=sessions.data??[]
 const listedPaths=new Set(listed.map(session=>session.path))
 const merged:Session[]=[...liveSessions.flatMap(live=>live.path&&!listedPaths.has(live.path)?[{path:live.path,id:live.path,cwd,firstMessage:live.title,messageCount:0,modified:new Date().toISOString()} satisfies Session]:[]),...listed]
 const visibleSessions=merged.flatMap(s=>`${s.name??''} ${s.firstMessage}`.toLowerCase().includes(search.toLowerCase())?[s]:[])
 return <><div className="panel-heading"><div><h1>会话</h1><p>保存在本机的项目对话。</p></div><Button className="primary" disabled={!online} onClick={()=>void changeSession({type:'new_session'}).catch(report)}><Icon name="plus"/>新会话</Button></div><Input className="search-input" placeholder="搜索会话" value={search} onChange={e=>setSearch(e.target.value)}/>{sessions.isLoading&&<Skeleton active paragraph={{rows:4}}/>}{sessions.error&&<p className="error-inline">{String(sessions.error)}</p>}<div className="session-rows">{visibleSessions.map(s=>{const working=liveSessions.some(live=>live.running&&live.path===s.path);return <div className={`session-row${working?' working':''}`} key={s.id}><Button disabled={!online} aria-busy={working} onClick={()=>void changeSession({type:'switch_session',sessionPath:s.path}).catch(report)}><Icon name="chats"/><span><strong>{s.name||s.firstMessage||'未命名会话'}</strong><small>{s.messageCount} 条消息 · {s.modified?new Date(s.modified).toLocaleString('zh-CN'):'刚刚'}</small></span>{working?<span className="session-working-indicator" title="正在工作" aria-hidden/>:<Icon name="arrow-up-right"/>}</Button>{listedPaths.has(s.path)&&<Button title="删除会话" disabled={!online} onClick={()=>setDeleting(s)}><Icon name="trash"/></Button>}</div>})}</div>{!merged.length&&<div className="empty-panel"><Icon name="chats"/><h3>还没有保存的会话</h3><p>发送第一条消息后，Pi 会自动保存。</p></div>}<Modal open={!!deleting} title="删除会话" onCancel={()=>setDeleting(null)} onOk={()=>void remove()} okText="删除" cancelText="取消">删除后无法恢复：{deleting?.name||deleting?.firstMessage||'未命名会话'}</Modal></>
}
