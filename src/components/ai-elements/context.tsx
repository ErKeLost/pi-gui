...
export type ContextTriggerProps = ComponentProps<typeof Button> & { showPercentage?: boolean };

export const ContextTrigger = ({ children, showPercentage = true, ...props }: ContextTriggerProps) => {
  const { usedTokens, maxTokens } = useContextValue();
  const usedPercent = usedTokens / maxTokens;
    ...
    <HoverCardTrigger delay={0} closeDelay={0}>
      {children ?? (
        <Button type="button" variant="ghost" {...props}>
          {showPercentage && <span className="font-medium text-muted-foreground">
            {renderedPercent}
          </span>}
          <ContextIcon />
        </Button>
...
export const ContextInputUsage = ({
  ...
  const inputCost = modelId
    ...
    : undefined;
  const inputCostText = currencyFormatter.format(inputCost ?? 0);
...
export const ContextOutputUsage = ({
  ...
  const outputCost = modelId
    ...
    : undefined;
  const outputCostText = currencyFormatter.format(outputCost ?? 0);
...
export const ContextReasoningUsage = ({
  ...
  const reasoningCost = modelId
    ...
    : undefined;
  const reasoningCostText = currencyFormatter.format(reasoningCost ?? 0);
...
export const ContextCacheUsage = ({
  ...
  const cacheCost = modelId
    ...
    : undefined;
  const cacheCostText = currencyFormatter.format(cacheCost ?? 0);
