      ...
      <div className="session-roots-list">
        {attached.map(path => {
          ...
          return <div className="session-roots-row" key={path}>
            ...
            {home ? <span className="session-roots-home">主项目</span> : <Button className="session-roots-remove" aria-label={`移除 ${name}`} disabled={!online} onClick={() => void setSessionRoots(extras.filter(root => root !== path)).catch(report)}><Icon name="x" /></Button>}
          </div>
        })}
      </div>
      {available.length > 0 && <div className="session-roots-available">
        {available.map(project => <Button key={project.path} className="session-roots-add" disabled={!online} onClick={() => void setSessionRoots([...extras, project.path]).catch(report)}>
