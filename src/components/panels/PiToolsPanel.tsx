...
export function PiToolsPanel() {
  ...
  return <>
    ...
    <section className="settings-section settings-action-section"><h2><Icon name="chats" />会话操作</h2><div className="tool-action-grid"><Button variant="ghost" disabled={!online || running} onClick={() => void importSession().catch(report)}><Icon name="download" />导入 JSONL</Button><Button variant="ghost" disabled={!online} onClick={() => void terminalCommand("/share").catch(report)}><Icon name="share-network" />分享会话</Button><Button variant="ghost" disabled={!online} onClick={() => void rename().catch(report)}><Icon name="pencil-simple" />重命名</Button><Button variant="ghost" disabled={!online} onClick={() => void copyLast().catch(report)}><Icon name="copy" />复制最后回复</Button></div></section>
    <section className="settings-section settings-action-section"><h2><Icon name="stack" />模型范围与资源</h2><div className="tool-action-grid"><Button variant="ghost" disabled={!online || running} onClick={() => void terminalCommand("/scoped-models").catch(report)}><Icon name="funnel" />Scoped Models</Button><Button variant="ghost" disabled={!online} onClick={() => void command("/reload").then(() => gooeyToast.success("资源已刷新", { showTimestamp: false })).catch(report)}><Icon name="arrows-clockwise" />重新加载资源</Button><Button variant="ghost" disabled={!online} onClick={() => void terminalCommand("/hotkeys").catch(report)}><Icon name="keyboard" />快捷键</Button><Button variant="ghost" disabled={!online} onClick={() => void terminalCommand("/changelog").catch(report)}><Icon name="list" />变更记录</Button></div></section>
    <section className="settings-section settings-package-section"><h2><Icon name="package" />Pi Package</h2><div className="input-row"><Input value={packageName} onChange={event => setPackageName(event.target.value)} placeholder="npm:包名 或 git:仓库" aria-label="Pi Package 名称" /><Button variant="default" disabled={!online || running} onClick={() => void packageCommand("install").catch(report)}><Icon name="plus" />安装</Button></div><div className="tool-action-grid"><Button variant="ghost" disabled={!online || running} onClick={() => void packageCommand("update").catch(report)}><Icon name="arrows-clockwise" />更新全部</Button><Button variant="ghost" disabled={!online || running || !packageName.trim()} onClick={() => void packageCommand("remove").catch(report)}><Icon name="trash" />移除</Button></div></section>
