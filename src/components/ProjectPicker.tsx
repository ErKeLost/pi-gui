...
export function ProjectPicker({compact=false}:{compact?:boolean}){
 ...
 return <><Popover open={visible} onOpenChange={setVisible}><PopoverTrigger render={<Button className={compact?'project-chip':'project-switch'} disabled={busy}/> }><Icon name={workspaceMode==='home'?'desktop':'folder-simple'}/><span>{name}</span><Icon name="caret-up-down"/></PopoverTrigger><PopoverContent align="start" className="project-menu"><div className="project-menu-heading"><span>工作区</span><small>{projects.length} 个项目</small></div><Button className={`project-home-option ${workspaceMode==='home'?'selected':''}`} aria-current={workspaceMode==='home'?'true':undefined} disabled={busy||!homeDir} onClick={()=>void chooseHome()}><Icon name="desktop"/><span className="project-option-copy"><strong>不在目录中工作</strong><small title={homeDir}>{homeDir||'用户主目录'}</small></span>{workspaceMode==='home'&&<span className="project-option-active">当前</span>}</Button><label className="project-search"><Icon name="magnifying-glass"/><Input autoFocus aria-label="搜索项目" placeholder="搜索项目" value={search} onChange={e=>setSearch(e.target.value)}/></label><div className="project-options">{visibleProjects.map(p=><div key={p.path} className={`project-option ${workspaceMode==='project'&&p.path===cwd?'selected':''}`}><Button aria-current={workspaceMode==='project'&&p.path===cwd?'true':undefined} className="project-option-main" disabled={busy} onClick={()=>void choose(p.path)}><Icon name="folder-simple"/><span className="project-option-copy"><strong>{p.name}</strong><small title={p.path}>{p.path}</small></span>{workspaceMode==='project'&&p.path===cwd&&<span className="project-option-active">当前</span>}</Button><Button className="project-option-remove" aria-label={`从列表移除 ${p.name}`} disabled={busy} onClick={()=>{setDeleting(p);setVisible(false)}}><Icon name="trash"/></Button></div>)}</div>{visibleProjects.length===0&&<p className="project-empty">没有匹配的项目</p>}<Button className="add-project" onClick={()=>void addProjects().catch(report)}><Icon name="plus"/><span>添加项目</span></Button></PopoverContent></Popover><Modal open={!!deleting} title="移除工作区" onCancel={()=>setDeleting(null)} onOk={()=>void removeWorkspace()} okText="从列表移除" cancelText="取消" destructive><div className="remove-workspace-copy"><strong>{deleting?.name}</strong><code>{deleting?.path}</code><p>只会从工作区列表移除，不会删除磁盘上的项目文件。</p>{workspaceMode==='project'&&deleting?.path===cwd&&<p>{projects.length>1?'这是当前工作区，移除后会自动切换到另一个工作区。':'这是当前工作区，移除后会回到空的工作区状态。'}</p>}</div></Modal></>
}
