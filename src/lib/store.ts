...
export type Workspace = {
  ...
  runtimeTarget:RuntimeTarget; cwd:string; connectionId:string; homeDir:string; workspaceMode:WorkspaceMode; piVersion:string; panel:Panel; settingsPage:SettingsPage; error:string | null; draft:string;
  dialogs:UiRequest[]; notices:string[]; statuses:Record<string,string>; widgets:Record<string,string[]>; liveSessions:LiveSession[]; agents:AgentSnapshot | null; multiAgentEnabled:boolean;
...
export const useWorkspace=create<Workspace>((set)=>({
  telemetry:emptyTelemetry(),inspector:false,transcript:emptyTranscript(),state:null,connection:'offline',runtimeTarget:'unknown',cwd:'',connectionId:'',homeDir:'',workspaceMode:'project',piVersion:'',panel:'chat',settingsPage:'general',error:null,draft:'',dialogs:[],notices:[],statuses:{},widgets:{},liveSessions:[],agents:null,multiAgentEnabled:initialMultiAgentMode(),
  set:(patch)=>set(patch),
