...
export type Workspace = {
  ...
  dialogs:UiRequest[]; notices:string[]; statuses:Record<string,string>; widgets:Record<string,string[]>; liveSessions:LiveSession[];
  set: (patch: Partial<Omit<Workspace,'set'|'event'>>) => void; event:(event:Event)=>void;
}
export const useWorkspace=create<Workspace>((set)=>({
  telemetry:emptyTelemetry(),inspector:false,transcript:emptyTranscript(),state:null,connection:'offline',cwd:'',connectionId:'',homeDir:'',workspaceMode:'project',piVersion:'',panel:'chat',settingsPage:'general',error:null,draft:'',dialogs:[],notices:[],statuses:{},widgets:{},liveSessions:[],
